...
DataStack datastack;//数据栈，先进先出
QStackedWidget *stack;
pdfHeaderEdit *pdf;//继承QWidget
...
{
    ui->setupUi(this);
    this->setWindowTitle(QString::fromUtf8("\u8f6c\u77e9\u8f6c\u901f\u6d4b\u91cf\u4eea"));//转矩转速测量仪
    setWindowState(Qt::WindowMaximized);//最大化
    home=new homePage;//继承QWidget
...
{
    if(page1->state==Page_1::StartedState)//如果page1的运行状态开始了
    ...
    if(drawpage->status==DrawPage::STARTED)
    {
        ...
        torsionlist_draw->append(qAbs(torsion));
